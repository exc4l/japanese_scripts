...
def get_kanji_grid_wcounter(kanjilist,
                            counter,
                            font,
                            columns,
                            fontcolor='#000000',
    ...
    for k in range(len(kanjilist)):
        ...
        if kan in counter:
            ...
            if ckan >= 5:
                kanji = get_kanji_picto(
                    kan, font, fontcolor=fontcolor, backcolor='#2dc937')
            elif ckan == 4:
                kanji = get_kanji_picto(
                    kan, font, fontcolor=fontcolor, backcolor='#99c140')
            elif ckan == 3:
                kanji = get_kanji_picto(
                    kan, font, fontcolor=fontcolor, backcolor='#e7b416')
            elif ckan == 2:
                kanji = get_kanji_picto(
                    kan, font, fontcolor=fontcolor, backcolor='#db7b2b')
            else:
                kanji = get_kanji_picto(
                    kan, font, fontcolor=fontcolor, backcolor='#cc3232')
        else:
            kanji = get_kanji_picto(
                kan, font, fontcolor=fontcolor, backcolor=backcolor)
        img.paste(kanji, (k * size % w, (k * size // w) * size))
    return img


def get_header(grade, font, size, columns,
               fontcolor='#000000', background='#FFFFFF',
               padding_size_top=30, padding_size_bot=50):
    size = size
    w = size * columns
    h = size
...
def get_stats(counter, unknown_kanji, total, font, size, columns):
    ...
    draw = ImageDraw.Draw(temp)
    known_percen = 100 * (total - len(unknown_kanji)) / total
    temp_str = (f'Unknown Kanji: {len(unknown_kanji)} '
                f'({known_percen:.2f}% known)')
    draw.text(
        (size, 0), temp_str, font=font, fill=fontcolor)
    tempimg = get_vert_cat(tempimg, temp)
    return tempimg


@click.command(context_settings=CONTEXT_SETTINGS)
@click.option('--print-unknown-kanji', '-P',
              is_flag=True,
              help='print unknown kanji in the grading')
def main(print_unknown_kanji):
    ...
    if bool(known_kanji):
        ...
        test = get_stats(kanji_freq, unknown_kanji, total,
                         headerfont, headerfont.size, columns)
