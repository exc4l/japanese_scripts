...
def get_stats(counter, unknown_kanji, total, font, size, columns):
    ...
    temp_str = f"Unknown Kanji: {len(unknown_kanji)} " f"({known_percen:.2f}% known)"
    draw.text((size, 0), temp_str, font=font, fill=fontcolor)
    tempimg = get_vert_cat(tempimg, temp)
    return tempimg
...
def main(print_unknown_kanji):
    ...
    if bool(known_kanji):
        ...
        test = get_stats(
            kanji_freq, unknown_kanji, total, headerfont, headerfont.size, columns
        )
