...
def html_processing(booklist, max_img_height):
    ...
    for bo in tqdm(booklist, ascii=True, desc="HTML Processing"):
        if not os.path.isfile(bo + "\\" + os.path.basename(bo) + ".html"):
            ...
            with open(
                bo + "\\" + os.path.basename(bo) + ".html", "w", encoding="utf-8"
            ) as wr:
...
def kanji_processing(booklist):
    ...
    repl_known_words = {w: f'<span class="knownword">{w}</span>' for w in known_words}
    pattern_known = kana.pattern_create(repl_known_words)
    repl_kanji = {k: f'<span class="knownkanji">{k}</span>' for k in known_kanji}
    ...
    for novel in tqdm(booklist, ascii=True, desc="Marking Files"):
        ...
        repl_unknown_kanji = {
            k: f'<span class="unknownkanji">{k}</span>' for k in unknown_kanji
        }
        pattern_unknown_kanji = kana.pattern_create(repl_unknown_kanji)
        repl_tagger = {
            t: f'<span class="taggerword">{t}</span>'
            for t in uniq_words
            if t in known_words or kana.contains_lemma(t, known_words, tagger)
        }
        ...
        if repl_tagger:
            ...
            bookhtml = kana.pattern_replacement(raw_book, pattern_tagger, repl_tagger)
            # bookhtml = kana.pattern_replacement(bookhtml,
            #                                     pattern_tagger, repl_tagger)
        pbar.update(25)
        bookhtml = kana.pattern_replacement(bookhtml, pattern_known, repl_known_words)
        pbar.update(25)
        bookhtml = kana.pattern_replacement(bookhtml, pattern_kanji, repl_kanji)
        pbar.update(25)
        bookhtml = kana.pattern_replacement(
            bookhtml, pattern_unknown_kanji, repl_unknown_kanji
        )
        with open(
            f"{novel}/Marked_{os.path.basename(novel)}.html", "w", encoding="utf-8"
        ) as wr:
            wr.write(bookhtml)
        pbar.close()
    return None
...
def split_creation(booklist, split):
    ...
    for novel in tqdm(booklist, ascii=True, desc="Splitting Books"):
        if os.path.isfile(f"{novel}/Marked_{os.path.basename(novel)}.html"):
            with open(
                f"{novel}/Marked_{os.path.basename(novel)}.html", "r", encoding="utf-8"
            ) as file:
                raw_book = file.read()
        else:
        ...
        for i in range(len(pages)):
            with open(
                (f"{novel}/{splitdir}/{i:0>3d}_" f"{os.path.basename(novel)}.html"),
                "w",
                encoding="utf-8",
            ) as wr:
...
def report_function(booklist):
    ...
    for novel in tqdm(booklist, ascii=True, desc="Creating Report"):
        ...
        known10plusper = int(
            100 * len(n10plus.intersection(known_kanji)) / len(n10plus)
        )
        add_data = [
            {
...
@click.option(
    "--do-kanji", "-K", help="mark known words/kanji in the html", is_flag=True
)
@click.option(
    "--bookdir",
    "-i",
...
@click.option(
    "--interactive",
    is_flag=True,
    help=("Select which books to process. Doesn't affect " "mobi extraction."),
)
def main(
    extract_mobi,
    force_mobi,
