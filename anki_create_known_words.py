...
def main(no_kanjigrid, user):
    ...
    for cards in card_list:
        ...
        write_to_file_text = write_to_file_text + \
            f'card model {card} found:' + '\n'
        print(len(sellist))
    ...
    if not DISREGARD_OLD_KNOWN:
        if os.path.isfile(kw_path):
            ...
            previous_known = [
                w for w in previous_known if not kana.is_single_kana(w)]
    ...
    write_to_file_text = write_to_file_text + \
        f'found a total of {len(muniq)} words' + '\n'
    write_to_file_text = write_to_file_text + \
        f'with a total of {len(uniqK)} unique kanji' + '\n'
    ...
    add_data = [{'Date': current_date, 'Time': current_time,
                 'Words': len(muniq), 'Kanji': len(uniqK)}]
